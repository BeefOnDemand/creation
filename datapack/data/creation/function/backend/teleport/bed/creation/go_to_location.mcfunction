...
$execute at @s as @a[distance=..1.5,sort=furthest,predicate=creation:on_creation_bed,predicate=!creation:sneaking,predicate=creation:creation_user] in $(dimension) if block $(x) $(y_bed) $(z) minecraft:yellow_bed run tp $(x) $(y) $(z)
