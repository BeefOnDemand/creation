...
execute if score [Creation] cr.admin.PlayersEnabledByDefault matches 1 run tag @s[predicate=!creation:creation_user] add cr.tag.CreationUser
...
execute unless score @s cr.admin.PackUpdated = [Creation] cr.admin.PackUpdated if score @s cr.admin.PackInstalled matches 1 run scoreboard players set @s cr.check.ConditionsPassed 1
execute if score @s cr.check.ConditionsPassed matches 1 run scoreboard players operation @s cr.admin.PackUpdated = [Creation] cr.admin.PackUpdated
